...
Rectangle {
    ...
    // Show the brush guide whenever the mouse is over the graph and we're
    // not panning (right-click held). This covers both draw and erase.
    BrushGuide {
        visible: gi.ma.containsMouse && !(gi.ma.pressedButtons & Qt.RightButton)
        mouseArea: gi.ma
        graphView: view
