...
Rectangle {
    ...
    readonly property rect _confirmedScreenRect: {
        ...
        if (!_hasSelection || a.width <= 0 || a.height <= 0) return Qt.rect(0, 0, 0, 0)
    ...
    readonly property int _hovIdx:
            gi.ma.containsMouse && !gi.ma.pressed && root.backend.pointCount > 0
        ? graphUtils.nearestPointIndex(gi.ma.mouseX, gi.ma.mouseY, 15)
        : -1
    ...
    Component {
        ...
        PlotPoint { color: root.selectedColor }
    }
    ...
    function recenter() { graphUtils.recenter() }
    ...
    GraphsView {
        ...
        theme: GraphsTheme {
            ...
            grid.mainColor: "#b7b7b7"; grid.mainWidth: 1.5
            grid.subColor:  "#CECECE"; grid.subWidth:  1
    ...
    BrushGuide {
        ...
        mouseArea: gi.ma; graphView: view; guideSize: root.brushSize
    }

    GuideOverlay {
        anchors.fill: view
        visible: root.showGuides && gi.ma.containsMouse && !root.selectionMode
        mouseArea: gi.ma; graphView: view; xAxis: xA; yAxis: yA
    }
    ...
    GraphInteractions {
        ...
        gu: graphUtils; gv: view; xAxis: xA; yAxis: yA
    }

    Rectangle {
        visible: gi.isDraggingSelection
        x: gi.selectionDragRect.x;     y: gi.selectionDragRect.y
        width: gi.selectionDragRect.width; height: gi.selectionDragRect.height
    ...
    Rectangle {
        ...
        x: root._confirmedScreenRect.x;          y: root._confirmedScreenRect.y
    ...
    Rectangle {
        ...
        anchors.top: view.top; anchors.right: view.right; anchors.margins: 10
        ...
        ColumnLayout {
            ...
            Label {
                ...
                color: {
                    const r = root._hovResidual
                    if (isNaN(r)) return colorPallete.text
                    if (r >  0.001) return "#c0522a"
                    if (r < -0.001) return "#2a7ac0"
                    return "#2a9d2a"
                }
            }
        }
