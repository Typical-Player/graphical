...
Item {
    ...
    readonly property rect selectionDragRect: Qt.rect(Math.min(_selX, graphMouseArea.mouseX), Math.min(_selY, graphMouseArea.mouseY), Math.abs(graphMouseArea.mouseX - _selX), Math.abs(graphMouseArea.mouseY - _selY))
    readonly property bool isDraggingSelection: root.selectionMode && graphMouseArea.pressed && (graphMouseArea.pressedButtons & Qt.LeftButton) && _wasDrag

    property real _selX: 0
    property real _selY: 0
    property bool _wasDrag: false

    property real _pinchXMin: 0
    property real _pinchXMax: 0
    property real _pinchYMin: 0
    property real _pinchYMax: 0
    ...
    PinchArea {
        ...
        onPinchStarted: {
            root._pinchXMin = root.xAxis.min;
            root._pinchXMax = root.xAxis.max;
            root._pinchYMin = root.yAxis.min;
            root._pinchYMax = root.yAxis.max;
        }

        onPinchUpdated: function (pinch) {
            const area = root.gv.plotArea;
            const cx = root._pinchXMin + ((pinch.center.x - area.x) / area.width) * (root._pinchXMax - root._pinchXMin);
            const cy = root._pinchYMax - ((pinch.center.y - area.y) / area.height) * (root._pinchYMax - root._pinchYMin);

            const factor = 1.0 / Math.max(pinch.scale, 0.01);

            root.xAxis.min = cx + (root._pinchXMin - cx) * factor;
            root.xAxis.max = cx + (root._pinchXMax - cx) * factor;
            root.yAxis.min = cy + (root._pinchYMin - cy) * factor;
            root.yAxis.max = cy + (root._pinchYMax - cy) * factor;
        }
        ...
        MouseArea {
            ...
            onPositionChanged: function (mouse) {
                if (!pressed)
                    return;
                if (!root.isMobile && (pressedButtons & Qt.RightButton)) {
                    const area = root.gv.plotArea;
            ...
            onReleased: function (mouse) {
                if (mouse.button !== Qt.LeftButton || !root.selectionMode)
                    return;
                if (root._wasDrag)
