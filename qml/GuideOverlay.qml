...
Item {
	...
	readonly property real clampedX: Math.max(root.graphView.plotArea.x,
		Math.min(root.graphView.plotArea.x + root.graphView.plotArea.width, root.mouseX))

	readonly property real clampedY: Math.max(root.graphView.plotArea.y,
		Math.min(root.graphView.plotArea.y + root.graphView.plotArea.height, root.mouseY))
	...
	Rectangle {
		id: xLabelBg
		x: Math.max(root.graphView.plotArea.x,
			Math.min(root.graphView.plotArea.x + root.graphView.plotArea.width - width,
				root.clampedX - width / 2))
	...
	Rectangle {
		...
		y: Math.max(root.graphView.plotArea.y,
			Math.min(root.graphView.plotArea.y + root.graphView.plotArea.height - height,
				root.clampedY - height / 2))
