...
class pointprocessing : public QObject {
    ...
    Q_PROPERTY(Progress selectionProgress READ selectionProgress NOTIFY selectionProgressChanged FINAL)
    ...
    Q_PROPERTY(bool hasSelectionResult READ hasSelectionResult NOTIFY selectionResultChanged FINAL)
    Q_PROPERTY(QString selectionResultEquation READ selectionResultEquation NOTIFY selectionResultChanged FINAL)
    Q_PROPERTY(SidebarResult selectionResultMatrices READ selectionResultMatrices NOTIFY selectionResultChanged FINAL)
    Q_PROPERTY(
        PlotTypes::PlotType selectionResolvedFitType READ selectionResolvedFitType NOTIFY selectionResultChanged FINAL)
...
public:
    ...
    [[nodiscard]] Progress selectionProgress() const;
    ...
    Q_INVOKABLE [[nodiscard]] QList<QPointF> allPoints() const;

    Q_INVOKABLE void setAllPoints(const QList<QPointF> &points);

    Q_INVOKABLE void keepOnlySelection();
    ...
    Q_INVOKABLE void setSelectionPoints(const QList<QPointF> &points);

    Q_INVOKABLE void clearSelection();

    Q_INVOKABLE double evaluateSelectionFitAt(double x) const;

    [[nodiscard]] bool hasSelectionResult() const;

    [[nodiscard]] QString selectionResultEquation() const;

    [[nodiscard]] SidebarResult selectionResultMatrices() const;

    [[nodiscard]] PlotTypes::PlotType selectionResolvedFitType() const;

    [[nodiscard]] QLineSeries *selectionFitSeries() const;

    void setSelectionProgress(Progress p);

private slots:
    void onWorkerFinished(const Result &result);

    void onSelectionWorkerFinished(const Result &result);

    void onWorkerError(const QString &err);

    void onWorkerCanceled();

    void onSelectionWorkerCanceled();
...
signals:
    ...
    void requestSelectionRun(const QList<QPointF> &points, PlotTypes::PlotType plotType, bool useFractions);

    void selectionResultChanged();
...
private:
    ...
    void fireSelectionWorker();

    void resampleSelectionFitSeries(double xMin, double xMax) const;
    ...
    workerprocessing *_selectionWorker{};
    QThread *_selectionWorkerThread{};
    QTimer *_selectionDebounceTimer{};
    QLineSeries *_selectionFitSeries{};

    QList<QPointF> _selectionPoints{};
    bool _hasSelectionResult{false};
    bool _selectionPendingRestart{false};
    QString _selectionResultEquation{};
    SidebarResult _selectionSdRes{};
    double _selBA{}, _selBB{}, _selBC{};
    PlotTypes::PlotType _selectionResolvedFitType{};
    Progress _selectionProgress{NOTFIRED};
};
